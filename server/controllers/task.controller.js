...
module.exports = async function createTask(req, res) {
    ...
    if (data.developer) {
        let dev = await User.findOneAndUpdate({ email: data.developer }, { $push: { currentTasks: task._id } }, err => {
            if (err) handleError(err, res);
        });
...
module.exports = function getMyTasks(req, res) {
    User
        ...
        .exec(async (err, user) => {
            ...
            for (let taskId of user.currentTasks) {
                await (await Task.findById(taskId)).execPopulate((err, task) => {
                    if (err) return handleError(err, res);
                    tasks.push(task);
                });
            }
